...
def conv_tri_torch(image, r):
    ...
    f = torch.tensor(f,dtype=torch.float32)[None, None, ...]
...
def get_nms(edge_pred, binary_threshold=140):
...
def get_NLE(images):
    equalized_image = exposure.equalize_hist(images)
    equalized_image = torch.from_numpy(equalized_image)
    equalized_image.unsqueeze_(0).unsqueeze_(0)
    structure = get_nms(equalized_image)
    structure.squeeze_(0).squeeze_(0)
    return structure
